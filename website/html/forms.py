...
class RegisterExperimentForm(forms.ModelForm):
  ...
  expe_name = forms.CharField(label="Experiment name", error_messages={'required': 'Enter a experiment name'},
                              required=True)
  researcher_name = forms.CharField(label="Researcher name", error_messages={'required': 'Enter a researcher name'},
                                    required=True)
  researcher_address = forms.CharField(label="Name and address of researcher's home institution", error_messages={
    'required': 'Enter a Name and address of researchers home institution'}, required=True)
  researcher_email = forms.CharField(label="Researcher's email address", widget=forms.EmailInput(
  ...
  researcher_institution_name = forms.CharField(label="Name of home institution's IRB officer or contact person",
                                                error_messages={
                                                  'required': 'Name of home institutions IRB officer or contact person'},
                                                required=True)
  ...
  goal = forms.CharField(label="What is the goal of your research experiment? What do you want to find out?",
                         ...
                         error_messages={'required': 'Enter the goal of your research experiment'}, max_length=256,
                         required=True)
...
class GeneralSensorAtributesForm(forms.ModelForm):
  ...
  frequency = forms.IntegerField(label='i. How often will you need to access the sensor data? Once every', min_value=1,
                                 widget=forms.NumberInput(attrs={'class': 'form-control'}), initial=1, required=False)
  ...
  frequency_other = forms.CharField(label="Other:", required=False, widget=forms.TextInput(
    attrs={'class': 'form-control', 'placeholder': 'Please provide any additional information that you would like'}))
  P_CHOICES = (('full', 'Full Precision'), ('truncate', 'Truncate'),)
  precision = forms.ChoiceField(label='ii. How precise do you need the data to be?', widget=forms.Select(),
                                choices=P_CHOICES, initial='full', required=False)
  truncation = forms.IntegerField(label='If truncation, choose the number of decimals to keep', min_value=1,
                                  widget=forms.NumberInput(attrs={'class': 'form-control'}), required=False, initial=1)
  precision_other = forms.CharField(
    label="A level of data precision that we currently do not support? Please elaborate:", required=False,
    widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 1,
                                 'placeholder': 'Please provide any additional information that you would like'}))
  goal = forms.CharField(label="iii. What will this sensor used for?", widget=forms.Textarea(
    attrs={'class': 'form-control', 'rows': 1, 'placeholder': 'Enter the goal of your Experiment'}),
                         error_messages={'required': 'Enter the goal of your research experiment'}, max_length=256,
                         required=False)
...
class BatteryForm(GeneralSensorAtributesForm):
  ...
  battery = forms.ChoiceField(choices=TRUE_FALSE_CHOICES, label="Battery", widget=forms.Select(), required=False,
                              initial=False)
...
class CellularForm(GeneralSensorAtributesForm):
  ...
  cellular = forms.ChoiceField(choices=TRUE_FALSE_CHOICES, label="Cellular", widget=forms.Select(), required=True,
                               initial=False)
  network_roaming = forms.BooleanField(
    label="network_roaming (returns true if the device is considered roaming on the current network, for GSM purposes)",
    required=False)
  ...
  network_operator = forms.BooleanField(
    label="network_operator (returns the numeric name, MCC+MNC, of current registered operator. Note: MCC+MNC  identify a unique operator)",
    required=False)
  network_operator_name = forms.BooleanField(
    label="network_operator_name (returns the alphabetic name of current registered operator)", required=False)
  network_type = forms.BooleanField(
    label="network_type (returns the radio technology, or network type, currently in use on the device)",
    required=False)
  service_state = forms.BooleanField(
    label="service_state (returns the state of cellular service: emergency call only, in service, out of service, or power off)",
    required=False)
...
class LocationForm(GeneralSensorAtributesForm):
  ...
  location_provider_enabled = forms.BooleanField(
    label="location_provider_enabled (check if one of the providers is enabled)", required=False)
...
class SignalStrengthForm(GeneralSensorAtributesForm):
  ...
  signalstrength = forms.ChoiceField(choices=TRUE_FALSE_CHOICES, label="Signal Strength", widget=forms.Select(),
...
class WifiForm(GeneralSensorAtributesForm):
  ...
  scan_results = forms.BooleanField(
    label="scan_results (list of access points found during the most recent WiFi scan: list of information similar to connectionInfo)",
    required=False)
...
class GeniUserCreationForm(DjangoUserCreationForm):
  ...
  gen_upload_choice = forms.ChoiceField(label="",
                                        choices=((1, 'Generate key pairs for me'), (2, 'Let me upload my public key')))
  username = forms.CharField(label="Username", error_messages={'required': 'Enter a username'},
                             max_length=validations.USERNAME_MAX_LENGTH)
